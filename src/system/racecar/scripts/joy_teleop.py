...
class JoyTeleop:
    ...
    def run_command(self, command, joy_state):
        ...
        if cmd["type"] == "topic":
            ...
            if cmd["action_name"] in self.offline_actions:
                rospy.logerr(
                    "command {} was not played because the action " "server was unavailable. Trying to reconnect...".format(cmd["action_name"])
                )
            ...
            if cmd["service_name"] in self.offline_services:
                rospy.logerr(
                    "command {} was not played because the service " "server was unavailable. Trying to reconnect...".format(cmd["service_name"])
                )
            ...
            raise JoyTeleopException("command {} is neither a topic publisher nor an action or service client".format(command))
        ...
        if "message_value" in cmd:
            ...
            for mapping in cmd["axis_mappings"]:
                if len(joy_state.axes) <= mapping["axis"]:
                    rospy.logerr(
                        "Joystick has only {} axes (indexed from 0), but #{} was referenced in config.".format(len(joy_state.axes), mapping["axis"])
                    )
                    val = 0.0
                else:
    ...
    def run_service(self, c, joy_state):
        ...
        if not self.srv_clients[cmd["service_name"]](request):
            rospy.loginfo("Not sending new service request for command {} because previous request has not finished".format(c))
